...
async function buyMiles(req, res) {
    ...
    try {
        const transactionResponse = await axios.post(`http://customer:5002/customers/${id}/transactions`, transactionPayload);
        if (transactionResponse.status === 201) {
            const user = await axios.patch(`http://customer:5002/customers/${id}/miles/buy`, { miles });
...
async function useMiles(req, res) {
    ...
    const { miles } = req.body;
    try {
        const user = await axios.patch(`http://customer:5002/customers/${id}`, { "miles": miles });
        const transaction = {
        ...
        await axios.post(`http://customer:5002/transactions`, transaction);
        return res.status(201).json({
